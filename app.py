...
if menu == "3. Marcar Consulta":
    ...
    try:
        # CORREÇÃO: Aumentamos o limite para 4000 para carregar TODOS os horários e especialidades
        ...
        if res_vagas.data:
            ...
            for r in res_vagas.data:
                ...
                if m:
                    ...
                    # Normalização para evitar erros de maiúsculo/minúsculo ou espaços
                    vagas_limpas.append({
                        'id': r['id'], 
            ...
            with c1:
                ...
                if u_sel != "Selecione...":
                    ...
                    # Agora o sistema puxa as especialidades diretamente do que existe na unidade selecionada
...
# TELAS ADMINISTRATIVAS
    ...
    if verificar_senha():
        ...
        if menu == "1. Cadastro de Médicos":
            ...
            especialidades = sorted(["Cardiologia", "Clinica", "Dermatologia", "Endocrinologia", "Endocrinologia - Diabete e Tireoide", "Fonoaudiologia", "Ginecologia", "Nefrologia", "Neurologia", "Neuropsicologia", "Nutricionista", "ODONTOLOGIA", "Oftalmologia", "Ortopedia", "Otorrinolaringologia", "Pediatria", "Pneumologia", "Psicologia", "Psiquiatria", "Urologia"])
            with st.form("f_med"):
            ...
            if res.data:
                ...
                for idx, r in enumerate(res.data):
                    ...
                    relat.append({"Nº": idx+1, "Data": dt.strftime('%d/%m/%Y %H:%M'), "Médico": m.get('nome'), "Paciente": pac if pac else "Livre", "Status": r['status'], "sort": r['data_hora']})
                st.data_editor(pd.DataFrame(relat).sort_values(by="sort").drop(columns=["sort"]), use_container_width=True, hide_index=True)
            ...
            if res.data:
                df_e = pd.DataFrame([{'id': r['id'], 'info': f"{pd.to_datetime(r['data_hora']).strftime('%d/%m/%Y %H:%M')} - {r.get('MEDICOS', {}).get('nome', 'N/I')}"} for r in res.data])
            ...
            if res_m.data:
                ...
                if sel_m != "Selecione..." and st.button("EXCLUIR"):
                    supabase.table("CONSULTAS").delete().eq("medico_id", lista_m[sel_m]).execute()
                    supabase.table("MEDICOS").delete().eq("id", lista_m[sel_m]).execute()
