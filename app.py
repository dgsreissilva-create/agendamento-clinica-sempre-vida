...
# ============================================
# TELA 4 - RELATÓRIO DE CONSULTAS FUTURAS
# (BUSCA INVERTIDA - VERSÃO ESTÁVEL)
# ============================================
    ...
    if verificar_senha():
        ...
        if not dados:
            st.info("Não há registros recentes.")
            st.stop()
        ...
        for r in dados:
            ...
            try:
                ...
                status = r.get("status")
                data_raw = r.get("data_hora")

                if not status or not data_raw:
                    continue

                dt_vaga = pd.to_datetime(data_raw, errors="coerce")

                if pd.isna(dt_vaga):
                    continue

                # Remove timezone se existir
                if dt_vaga.tzinfo is not None:
                    dt_vaga = dt_vaga.tz_localize(None)

                # Apenas Marcadas e Futuras
                if status != "Marcada":
                    continue
        ...
        renderizar_quadro("🏢 Quadro 2 - Carijós", unidades_q2)
